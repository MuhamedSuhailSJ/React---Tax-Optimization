...
const fetchCoinDetails = (coin) => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      const foundCoin = coins.find((c) => c.coin === coin);
      if (foundCoin) {
        resolve(foundCoin);
      } else {
...
app.get("/coins/:coin", (req, res) => {
  fetchCoinDetails(req.params.coin)
    .then((coin) => {
      res.json(coin);
    })
    .catch((error) => {
...
app.get("/capital-gains", (req, res) => {
  res.send(capitalGains.capitalGains);
});
